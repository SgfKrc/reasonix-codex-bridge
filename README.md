...
REASONIX_MODEL_REF = "your-inventory-name/deepseek-flash"
```
...
```powershell
reasonix subagent create deepseek-worker --scope global --model "your-inventory-name/deepseek-flash" --prompt-file .\prompts\deepseek-worker-prompt.md
...
`REASONIX_EXE`, `REASONIX_ROOT`, `REASONIX_SUBAGENT`, and `REASONIX_MODEL_REF` are configurable. The model reference is hard-locked to `your-inventory-name/deepseek-flash`; changing it makes the server exit with code 2. `REASONIX_ADD_DIRS` may contain additional allowed roots separated by the platform path delimiter.
